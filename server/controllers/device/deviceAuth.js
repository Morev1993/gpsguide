...
exports.login = function (req, res, next) {
  ...
  if (!authCode) {
    return res.status(422).send({ error: 'Auth code is empty.' });
  }

  // Return error if no email provided
  if (!model) {
    return res.status(422).send({ error: 'Model is empty.' });
  }
  ...
  Device.findOne({ authCode }, (err, device) => {
    if (err) { return next(err); }

      // If user is not unique, return error
    if (!device) {
      return res.status(422).send({ error: 'That authCode gone.' });
    }
